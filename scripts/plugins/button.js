(function() {
    FX.plugins["button"] = (function(id, option) {
        ...
        function addButtonListener(demo, btID) {
            ...
            //打开全部画面
            function openAllPictures(stateId, type) {
                var demo = $('#' + stateId);
                $.each(demo.children(), function(index, item) {
                    if ($(item).attr('title') && $(item).attr('title').substring(0, 2) === '画面') {
                        if ($(item).css('display') === 'none') {
                            ...
                            PlayInteractiveInPopupContent($(item), type);
                        }
                    }
                })
            }
            ...
            //关闭画面 全部/指定
            function closePictures(stateId, type) {
                var demo = $('#' + stateId);
                //关闭全部画面
                if (type === '0') {
                    $.each(demo.children(), function(index, item) {
                        if ($(item).attr('title') && $(item).attr('title').substring(0, 2) === '画面') {
                            if ($(item).css('display') !== 'none') {
                                ...
                                ResetInteractiveInPopupContent($(item), type);
                                // ShowUpButton();
                            }
                        }
                    })
                ...
                //关闭指定画面 
                    ...
                    closeIMGButton(demo.children()[parseInt(type) - 1]);
                }
            }
            ...
            function ShowByPopupContent() {
                ...
                if (displayFirstImage === "true" && displayCloseButton === "true") {
                    addCloseImgListener(demoSlide);
                }
            }
            ...
            demo.on(goToPageEvent, function(e) {
                ...
                if (gotoStateAction === "true" && statetype === "0") //打开全部画面
                    openAllPictures(stateuid, statetype);
                else if (closeStateAction === "true") //关闭画面
                    closePictures(stateuid, statetype);
