    ...
    FX.plugins["animation"] = (function(id, option) {
        ...
        Animation.prototype.reset = function(option) {
            ...
            if (!$.isEmptyObject(this.pageObj)) {
                ...
                if (this.pageObj.autoAnimationTreeArray.length > 0) {
                    $.each(this.pageObj.autoAnimationTreeArray, function(autonAnimationIndex, autoAnimationItem) {
                ...
                if (this.pageObj.clickAnimationTreeArray.length > 0) {
                    ...
                    $(page).on('click', function() {
                        ...
                        var animationId = ele.id;
                        var demo = $("#" + animationId).children('div')[0];
        ...
        Animation.prototype.destroy = function() {
            ...
            if (!$.isEmptyObject(this.pageObj)) {
                $.each(this.pageObj.orderArray, function(index, item) {
                    var demo = $("#" + item.id).children('div')[0];
                ...
                if ($(item).parents("div[title='PopupContent']").length === 0 || type === 'pop') {
                    ...
                    $.each(animations, function(animationIndex, animationItem) {
                        ...
                        if (originalAnimationArray instanceof Array) {
                            originalAnimationArray.push({ id: $(item).attr('id'), animations: animationItem, uId: animationItem.uid, order: i, groupId: gid, type: type, opacity: opacity });
                        } else {
                            originalAnimationArray = [{ id: $(item).attr('id'), animations: animationItem, uId: animationItem.uid, order: i, groupId: gid, type: type, opacity: opacity }];
                        }
                    })
                }
            })
        ...
        function playAnimationTree(root) {
            ...
            if (is_ios()) {
                if (value.playTime == 0) {
                    value.playTime = 0.1
                    if (value.playDelay >= 0) {
                        value.playDelay = value.playDelay - 0.1
                    }
                }
            }
            ...
            demo.offsetWidth = demo.offsetWidth;
