(function () {
	FX.plugins["button"] = (function (id, option) {
											...
											if (demoX[0].style.display === "inline") {
												...
												if (animateObj !== undefined) {
													...
													for (var idx = 0; idx < iLenth; idx++) {
														...
														var stopId = $(animateObj[idx]).data("stopId");
														if (stopId !== undefined) {
															clearTimeout(stopId);
															$(animateObj[idx]).data("stopId", undefined);
														}

														if ($(demoAnimation).is(":animated")) $(demoAnimation).stop(true, true);
														if (!$(demoAnimation).data("itemTop")) $(demoAnimation).data("itemTop", demoAnimation.offsetTop);
														if (!$(demoAnimation).data("itemLeft")) $(demoAnimation).data("itemLeft", demoAnimation.offsetLeft);
													...
													function playAnimation(iAni, waitTime, nSynWithFirstAni) {
														...
														if (dataAni.animate.type.charAt(dataAni.animate.type.length - 1) === '2') {
															...
															dataAni.Demo.data("stopId", stopId);
														}
													}
		...
		function UnTopPopupContent(showNode) {
			var showchildnodes = showNode.find("div[title='Animation']");
			for (var i = 0; i < showchildnodes.length; i++) {
				var curAniNode = showchildnodes.eq(i);
				var divchild = $(showchildnodes[i].children[0]);
				...
				if (dataAnis0.effect === 'fade' && dataAnis0.type.indexOf('toAppearance-') >= 0) {
					...
					if (dataAnis0.effect === 'fade' ||
						...
						dataAnis0.effect === 'flip'
                    ) {
						divchild.css('display', 'none');
					}
				} else if (triggerEvent === '1') {
					var curId = curAniNode.attr('id');
					var aniObj = window.fx.getItemById(curId);
					aniObj.reset();
				}
			}
